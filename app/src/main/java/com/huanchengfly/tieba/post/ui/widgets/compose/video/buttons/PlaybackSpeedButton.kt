...
fun PlaybackSpeedBottomSheetButton(
  ...
  PlaybackSpeedControl(player) {
    TextButton(
      ...
      enabled = isEnabled && gestureState?.isEnabled ?: true,
      modifier = modifier,
      colors = colors,
    ...
    if (showBottomSheet) {
      content {
        ...
        gestureState?.autoHideControls(timeout = CONTROLS_VISIBILITY_TIMEOUT_MS / 2)
      }
    }
  }
}
...
@OptIn(ExperimentalMaterial3Api::class)
private val defaultPlaybackSpeedBottomSheet: @Composable PlaybackSpeedState.(() -> Unit) -> Unit = { onDismiss ->
...
private fun PlaybackSpeedSliderWithAdjusters(
  ...
  Row(
    ...
    PlaybackSpeedAdjusterButton(
      state = state,
      painter = painterResource(R.drawable.media3_icon_minus),
