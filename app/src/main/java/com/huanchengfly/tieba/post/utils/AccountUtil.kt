...
import com.huanchengfly.tieba.post.revival.SessionHealthEvaluator
...
object AccountUtil {
    ...
    private fun refreshAccountsState(): List<Account> {
        return findAll(Account::class.java).also {
    ...
    @JvmStatic
    fun getSessionHealth(account: Account? = getLoginInfo()): SessionHealth {
        return SessionHealthEvaluator.fromAccount(account)
    }

    @JvmStatic
    fun hasCompleteSession(account: Account? = getLoginInfo()): Boolean {
        return getSessionHealth(account).isComplete
    }

    fun getCookieSessionHealth(cookie: String?): SessionHealth {
        return SessionHealthEvaluator.fromCookie(cookie)
    }

    fun getWebOnlySessionHealth(cookie: String?): SessionHealth {
        return SessionHealthEvaluator.webOnly(cookie)
    }

    @JvmStatic
    fun switchAccount(context: Context, id: Int): Boolean {
        val account = refreshAccountsState().firstOrNull { it.id == id }
            ?: runCatching { getAccountInfo(id) }.getOrNull()
            ?: return false
        val switched = context.getSharedPreferences("accountData", Context.MODE_PRIVATE).edit()
        ...
        BackgroundWorkScheduler.ensureNotificationPolling(context)
        BackgroundWorkScheduler.refreshNotificationsNow(context)
        return switched
    }

    private fun clearUnreadNotifications(context: Context) {
        val notificationManager =
            context.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
        UnreadNotificationHelper.clear(context, notificationManager)
    }
    ...
    fun exit(context: Context) {
        ...
        val removedAccountId = account.id
        account.delete()
        CookieManager.getInstance().removeAllCookies(null)
        val accounts = refreshAccountsState()
        val nextAccount = accounts.firstOrNull { it.id != removedAccountId } ?: accounts.firstOrNull()
        if (nextAccount != null) {
